...
class Files(Prefetch):
	...
	def __init__(self):
		...
		with open(self.file_name, encoding="utf-8") as json_file:
			...
			json_file.close()
		csv_obj = s3_client.get_object(Bucket='augmentor-customer-data', Key='AXIADO-INTERESTS/AUGMENTOR_TRANSIENT3.csv')
		body = csv_obj['Body']
	...
	def decayFunc1(self, df, freq):
		...
		if diff.days <=183:
		    return math.ceil(freq * math.exp((math.log(183) - math.log(diff.days))*0.1025))
		elif diff.days <= 365:
	...
	def get_param(self):
		...
		s3_client.put_object(Body=json.dumps(self.__output_df), Bucket = 'augmentor-customer-data', Key = 'AXIADO-INTERESTS/idj.json')
		...
		try:
			...
			obj_ = s3_client.get_object(Bucket='augmentor-customer-data', Key='AXIADO-INTERESTS/obj_user_interest.json')
			...
			obj_ = s3_client.get_object(Bucket='augmentor-customer-data', Key='AXIADO-INTERESTS/decay_uid.json')
			data = obj_['Body'].read().decode('utf-8')
			...
			s3_client.put_object(Body=json.dumps(self.__natest), Bucket = 'augmentor-customer-data', Key = 'AXIADO-INTERESTS/obj_user_interest.json')
			s3_client.put_object(Body=json.dumps(self.__natest), Bucket = 'augmentor-customer-data', Key = 'AXIADO-INTERESTS/backup_uid.json')
			s3_client.put_object(Body=json.dumps(self.__decay_natest), Bucket = 'augmentor-customer-data', Key = 'AXIADO-INTERESTS/obj_user_interest_decay.json')
