...
class Files(Prefetch):
	...
	def __init__(self):
		...
		with open(self.file_name, encoding="utf-8") as json_file:
			...
			json_file.close()
		csv_obj = s3_client.get_object(Bucket='augmentor-customer-data', key = 'REUP-INTERESTSESTS/REUP_TRANSIENT.csv')
		body = csv_obj['Body']
	...
	def get_param(self, file_id):
		...
		s3_client.put_object(Body=json.dumps(test_interest_json), Bucket = 'augmentor-customer-data', Key = 'REUP-INTERESTS/testInterest.json')
		...
		s3_client.put_object(Body=json.dumps(res_df), Bucket = 'augmentor-customer-data', Key = 'REUP-INTERESTS/final_au_int_df_update.json')
